...
module GameState
	...
	def self.update_game_state(user, opponent, place_selection)
		...
		Log.instance.info("Adding #{place_selection} to list of spaces occupied by #{user.to_sym}")
		...
		# If not the winner, update the game state
			...
			Log.instance.info("Updating board to place #{user.piece} in #{place_selection}" )
